
--------------------------------
-- @module DirectionController
-- @extend Node

--------------------------------
-- @function [parent=#DirectionController] setEnabled 
-- @param self
-- @param #bool bool
        
--------------------------------
-- @function [parent=#DirectionController] onTouchEnded 
-- @param self
-- @param #cc.Touch touch
-- @param #cc.Event event
        
--------------------------------
-- @function [parent=#DirectionController] onTouchMoved 
-- @param self
-- @param #cc.Touch touch
-- @param #cc.Event event
        
--------------------------------
-- @function [parent=#DirectionController] setTouchRadius 
-- @param self
-- @param #float float
        
--------------------------------
-- @function [parent=#DirectionController] isEnabled 
-- @param self
-- @return bool#bool ret (return value: bool)
        
--------------------------------
-- @function [parent=#DirectionController] onTouchCancelled 
-- @param self
-- @param #cc.Touch touch
-- @param #cc.Event event
        
--------------------------------
-- @function [parent=#DirectionController] onTouchBegan 
-- @param self
-- @param #cc.Touch touch
-- @param #cc.Event event
-- @return bool#bool ret (return value: bool)
        
--------------------------------
-- overload function: create(cc.Node, cc.Node)
--          
-- overload function: create(string, string)
--          
-- @function [parent=#DirectionController] create
-- @param self
-- @param #string str
-- @param #string str
-- @return DirectionController#DirectionController ret (retunr value: pf.DirectionController)

--------------------------------
-- @function [parent=#DirectionController] DirectionController 
-- @param self
        
return nil
